...
RSpec.describe TypedEnums::Railtie do
  ...
  describe ".install_development_hooks" do
    ...
    before do
      ...
      allow(TypedEnums).to receive(:watch)
    end
    ...
    it "registers generation and starts watching in development" do
      ...
      expect(TypedEnums).to have_received(:watch)
    end
    ...
    it "does not start watching in production" do
      ...
      expect(TypedEnums).not_to have_received(:watch)
    end

    it "honors the watch configuration" do
      allow(Rails).to receive(:env).and_return(ActiveSupport::StringInquirer.new("development"))
      TypedEnums.configure { |config| config.watch_models_in_development = false }
