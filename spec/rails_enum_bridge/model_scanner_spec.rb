...
RSpec.describe RailsEnumBridge::ModelScanner do
  ...
  it "raises a clear error when the root model class is missing" do
    RailsEnumBridge.configure { |config| config.root_model_class = "MissingRecord" }

    expect { described_class.new.call }.to raise_error(RailsEnumBridge::ConfigurationError, /MissingRecord/)
  end
end
