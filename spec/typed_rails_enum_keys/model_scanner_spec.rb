...
RSpec.describe TypedRailsEnumKeys::ModelScanner do
  ...
  it "raises a clear error when the root model class is missing" do
    TypedRailsEnumKeys.configure { |config| config.root_model_class = "MissingRecord" }

    expect { described_class.new.call }.to raise_error(TypedRailsEnumKeys::ConfigurationError, /MissingRecord/)
  end
end
