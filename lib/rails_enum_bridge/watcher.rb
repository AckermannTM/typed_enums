...
module RailsEnumBridge
  class Watcher
    ...
    def initialize(root: Rails.root, logger: Rails.logger, listener_factory: nil, generator: RailsEnumBridge)
      @root = Pathname(root)
      @logger = logger
    ...
    def start
      ...
      unless watch_path.directory?
        logger&.warn("rails_enum_bridge: model watch path does not exist: #{watch_path}")
        return self
      end
      ...
      listener.start
      @started = true
      logger&.info("rails_enum_bridge: watching #{watch_path}")
      self
    end
    ...
    def regenerate(modified:, added:, removed:)
      ...
      logger&.info("rails_enum_bridge: model file changed, regenerating TypeScript enums")
      reload_application
      generate
    rescue StandardError => e
      logger&.error("rails_enum_bridge: watch regeneration failed: #{e.class}: #{e.message}")
    end
