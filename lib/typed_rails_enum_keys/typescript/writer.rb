  ...
  module TypeScript
    class Writer
      ...
      def ensure_output_dir
        ...
        raise Error, "Could not create typed_rails_enum_keys output directory #{output_dir}: #{e.message}"
      end

      def write_file(path, content)
        FileUtils.mkdir_p(path.dirname)
        path.write(content)
        logger&.info("typed_rails_enum_keys: wrote #{path}")
      rescue SystemCallError => e
      ...
      def remove_file(path)
        path.delete
        logger&.info("typed_rails_enum_keys: removed stale #{path}")
      rescue SystemCallError => e
