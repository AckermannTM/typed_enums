...
module TypedRailsEnumKeys
  class Watcher
    ...
    def initialize(root: Rails.root, logger: Rails.logger, listener_factory: nil, generator: TypedRailsEnumKeys)
      @root = Pathname(root)
      @logger = logger
    ...
    def start
      ...
      unless watch_path.directory?
        logger&.warn("typed_rails_enum_keys: model watch path does not exist: #{watch_path}")
        return self
      end
      ...
      listener.start
      @started = true
      logger&.info("typed_rails_enum_keys: watching #{watch_path}")
      self
    end
    ...
    def regenerate(modified:, added:, removed:)
      ...
      logger&.info("typed_rails_enum_keys: model file changed, regenerating TypeScript enums")
      reload_application
      generate
    rescue StandardError => e
      logger&.error("typed_rails_enum_keys: watch regeneration failed: #{e.class}: #{e.message}")
    end
